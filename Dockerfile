...
RUN	yum -y install wget make bzip2-devel gcc-c++ gcc-gfortran libX11-devel libicu-devel libxml2 \
  	...
  	libcurl-devel mesa-libGL-devel mesa-libGLU-devel mysql-devel Nlopt-devel \
...
RUN yum -y localinstall jdk-8u181-linux-x64.rpm
RUN alternatives --install /usr/bin/java jar /usr/java/latest/bin/java 200000 && \
	alternatives --install /usr/bin/javaws javaws /usr/java/latest/bin/javaws 200000 && \
	alternatives --install /usr/bin/javac javac /usr/java/latest/bin/javac 200000 && \
	rm -fr jdk-8u181-linux-x64.rpm
